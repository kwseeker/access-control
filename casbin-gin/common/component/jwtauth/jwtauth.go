...
// 认证失败，返回401
// 认证成功，接着调用责任链后续的中间件，可以通过 c.Get("userID").(string) 获取userID
// 可以通过向LoginHandler发送post json请求获取JWT的token, 后续请求需要将这个token放到请求头的Authentication字段，如：Authorization:Bearer XXX_TOKEN_XXX
